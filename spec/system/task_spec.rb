...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  let!(:task_second){ FactoryBot.create(:second_task,user: user) }
  let!(:task_third){ FactoryBot.create(:third_task,user: user) } 
  # ↑ task_second = FactoryBot.create(:second_task,user: user と意味がほぼ同じ
  describe do
    before do
    ...
    describe '一覧表示機能' do
      ...
      context 'タスクが作成日時も降順に並んでいる場合' do
        it '新しいタスクが一番上に表示される' do
          FactoryBot.create(:task, name: '最新task', user: user)
          visit tasks_path
